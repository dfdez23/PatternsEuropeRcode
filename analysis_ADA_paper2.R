...
pacman::p_load(readr,readxl,dplyr,tidyverse,archetypes,tidyr, hopkins, 
               compareGroups,vcd,pheatmap,ggplot2, kableExtra)

dir <- "/Users/dfmartinez/Library/CloudStorage/Dropbox/MyPapers/202510_ADASpatial_election_PaolaMarica_Paper2"
setwd(dir)

#All the code and object come from the file Analysis_ADA.R, which is paper 1, 
#so I comment everthing from that R code to separate it 
#from this code for paper2 (spatial analysis)

#I read everything from Analysis_ADA.R code (I made a copy in this new location)
load(file="./Rdata/preliminary.RData")

#Let's graph the ternary plots of some of the countries according to boundaries.
#Let's do, of instance: Scandinavian, center of Europe, & east of Europe


#ACTUALLY EVERYTHING IS DONE ALREADY IN Analysis_ADA.R. GO TO WORK THERE.
...
# 
# 
# #We do all countries now excepet those three: Spain, Hungary and Sweden
...
#           tern.axis.line = element_line(linewidth = 0.05),
#           plot.title = element_text(hjust = 0.5)) +
#     geom_point(aes(size = seatperc), alpha = 0.8, color = "black", fill = i, shape = 21) +
#     #geom_text(aes(label = party), size = 3, color = "black", hjust = 1.2) +
#     geom_text(aes(label = party), size = 3, color = "black", vjust = -1) +
#     scale_size_area(breaks = c(5, 10, 20, 40, 60, 80,100), limits = c(0, 100), max_size = 12, name = "% Seats") +
#     #labs(x = as.vector(whoADAare$family[1]), 
